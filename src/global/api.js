...
export default {
  install: function (Vue) {
    // 通用增删改查等,api是对应接口的前缀.如user/login,则api为user.非共用方法写在api的各自js中
    /**
     * 通用调用方法,只支持get和post
     * @param url 请求地址
     ...
     * @param fresh 是否刷新,判断为false时刷新,true不刷新
    ...
    /**
     * 通用单条数据删除,id只能是字符串,不可是对象或数组等
     */
    Vue.prototype.$removeName = function (api, id, handler, callback, fresh, freshTree) {
      MessageBox.confirm('此操作将删除所选数据, 是否继续?', '提示', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        type: 'warning'
      }).then(() => {
        return new Vue().$common(`${api}/removeName/${id}`, {}, 'get', '删除成功', handler, callback, fresh, freshTree);
      }).catch(() => {
        Message.info('已取消删除');
      });
    };
     ...
     * @param ids 是一个单纯的数组,而不是一个对象,虽然数组也是对象.例如[1,2],而不是{ids:[1,2]}
     */
    Vue.prototype.$removeNames = function (api, ids, handler, callback, fresh, freshTree) {
      MessageBox.confirm('此操作将删除所选数据, 是否继续?', '提示', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        type: 'warning'
      }).then(() => {
        return new Vue().$common(`${api}/removeNames`, ids, 'post', '删除成功', handler, callback, fresh, freshTree);
      }).catch(() => {
        Message.info('已取消删除');
      });
    };
     ...
     */
    Vue.prototype.$getByName = function (api, id, handler, callback, fresh, freshTree) {
      return new Vue().$common(`${api}/getByName/${id}`, {}, 'get', null, handler, callback, fresh, freshTree);
    };
    /**
